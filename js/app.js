//Add for margin
    ...
    var margin = {top: 20, right: 120, bottom: 20, left: 120}, //right: 120, left: 120
        width = 960 - margin.right - margin.left,             //width = 960 - margin.right - margin.left;
        height = 800 - margin.top - margin.bottom;

//End margin add

var radius = 960 / 2; //radius = 960 / 2;

var cluster = d3.layout.cluster()
    .size([360, radius - 120]); //.size([360, radius - 120]);
...
d3.json("data/teams_men.json", function(error, root) {
  ...
  var link = svg.selectAll("path.link")
    ...
    .enter().append("path")
      .attr("class", "link")
      .attr("d", diagonal); //diagonal

  var node = svg.selectAll("g.node")
      .data(nodes)
    .enter().append("g")
      .attr("class", "node")
      .attr("transform", function(d) { return "rotate(" + (d.x - 90) + ")translate(" + d.y + ")"; }); //added ; at end of expression
